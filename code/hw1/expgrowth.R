...
simExpGrowth <- function(init, gen) {

  n0 <- init # initial number of individuals
  max.n <- 1e6 # total (maximum) number of individuals before we quit (to guard against horrible slowdowns)
  max.gen <- gen # maximum number of generations we'll track
  ...
  if (file.out) dev.out(paste("exp_growth_", init, "_", gen, sep=""), output.type=output.type)
  split.screen(c(1,2))
  screen(1)
  ...
  cat("Fraction of dead populations after", max.gen, "generations =", length(end.res[end.res==0])/n.reps, " when starting with ", n0, "individuals\n")
}
